object ArrayManipulation{
    ...
    def sortArray(arr: Array[Int]) : Unit = {
        for (i<-0 until arr.size){
            for (j<-0 until arr.size - i - 1){
                if (arr(j) > arr(j+1)){
                    val tmp = arr(j)
                    arr(j) = arr(j+1)
                    arr(j+1) = tmp
    ...
    def matrixPrint(mat: Array[Array[Int]]) : Unit = {
        for (i <- Array.range(mat.size-1, -1, -1)){
            for (j <- Array.range(mat(i).size-1, -1, -1)){
                print(s"${mat(i)(j)} ")
            }
            println() 
        }
    }
    def printDivideBy3(mat: Array[Array[Int]]) : Unit = {
        val N = mat(0).size - 1
